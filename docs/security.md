...
**One public payload boundary.** HTML hydration state, JSON bodies, and navigation segments all pass through the same projector before serialization. Representation-specific DTOs are supported, but every one of them is held to the same rules.
...
**Response policy, on request.** Negotiation always adds the request headers that could change the representation to `Vary`. Beyond that, configuring `representation.cache` or `representation.securityHeaders` turns on a stage that applies `Cache-Control` (defaulting to `private, no-store`), `X-Content-Type-Options: nosniff`, a `Referrer-Policy`, and an optional nonce-based CSP. It never overwrites a header the host application already set. These are off until asked for, because turning them on changes every response an application sends; that default flips in the next breaking release.

**Standards-aware negotiation.** Media ranges are parsed in full — qualities, wildcards, exclusions, structured suffixes — and a route that cannot satisfy the request answers `406` with the media types it does offer, never with controller data.
