...
no migration to perform. Everything below is new capability that applications
may adopt independently.

The defaults that _should_ change — conservative caching and enforced payload
limits — are deliberately held back. Existing controller shapes also retain
their historical `Accept` behavior. The new explicit factories have no prior
wire contract, so they use correct standards-aware ranking immediately.
...
- **Payload diagnostics.** Values outside the configured serialization rules
  are logged with the property path and never the value.
...
request negotiates is built — passing a function defers the work entirely. The
function receives the request's `AbortSignal`, allowing data work to stop on a
...
Allowed headers are available at `context.headers['x-tenant-id']` as well as
the established `context['x-tenant-id']` alias. Reserved names such as `path`
remain isolated in the bag and cannot shadow URL context.

Both access forms remain supported. `useHeader()` and `useHeaders()` read the
bag and fall back to the top-level shape.
...
## Two supported JSON configuration styles

`jsonApi` remains supported and still serves page props as the JSON body.
`representation.json` is an additive alternative for applications that also
want limits, cache policy, security headers, or independent API DTOs.
...
A `@Render()` route that returns a string preserves the existing passthrough
contract. Because the string bypasses projection, payload limits, and response
policy, applications should only use it for already-safe, deliberate output.
...
// A plain route is also available when @Render() adds no value.
@Get('legacy')
